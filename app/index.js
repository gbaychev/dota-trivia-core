...
class EntryPoint {
    ...
    initEndpoints () {
        ...
        this.app.post('/', (req, res) => {
            if(req.session.state === undefined) {
                logger.error(`${req.ip} POST /: undefined session state`);
                res.statusCode = 400;
            ...
            if(answer === undefined) {
                logger.error(`${req.ip} ${req.session.id} POST /: no answer provided`);
                res.statusCode = 400;
                res.send('no answer provided');
                return;
            }            
            if(!(answer instanceof Array) || answer.some(component => typeof component !== 'string')) {
                logger.error(`${req.ip} ${req.session.id} POST /: malformed answer provided`);
                res.statusCode = 400;
            ...
            logger.error(`${req.ip} ${req.session.id} POST /: ()`, req.session.state);
    ...
    run () {
        this.itemStore.initialize()
            ...
            .catch(e => {
                logger.error(`Item store failed to initialize, error: ${e.message}, exiting`);
            });
    }
}
